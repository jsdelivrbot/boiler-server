boilerAdmin.directive('chartStatus2', function () {
    return {
        restrict: 'E',
        templateUrl: "/directives/chart_status2.html",
        controller: "ChartStatus2Controller",
        controllerAs: "s2Chart"
    };
}).controller("ChartStatus2Controller", function ($rootScope, $scope, $http) {
    var bChart = this;
    bChart.range = RUNTIME_RANGE_DEFAULT;
    bChart.keys = [];

    var domId = "chart_status2";
    var dataField = "status2_data";
    ...
    $rootScope.$watch('instants', function (newVal, oldVal) {
        ...
        if (!bChart.keys || bChart.keys.length <= 0) {
            var idx = 2;
            if (idx >= bChart.instants.length) {
                idx = bChart.instants.length - 1;
            }
    ...
    bChart.refreshChart = function (range, keys) {
        ...
        switch (range) {
            ...
            case RUNTIME_RANGE_WEEK:
                ...
                Ladda.create(document.getElementById('s2Chart' + range)).start();
                $http.post('/boiler_runtime_list/', postData).then(function (res) {
                    ...
                    Ladda.create(document.getElementById('s2Chart' + range)).stop();
                });
                break;
