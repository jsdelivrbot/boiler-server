boilerAdmin.directive('chartStatus3', function () {
    return {
        restrict: 'E',
        templateUrl: "/directives/chart_status3.html",
        controller: "ChartStatus3Controller",
        controllerAs: "s3Chart"
    };
}).controller("ChartStatus3Controller", function ($rootScope, $scope, $http) {
    var bChart = this;
    bChart.range = RUNTIME_RANGE_DEFAULT;
    bChart.keys = [];

    var domId = "chart_status3";
    var dataField = "status3_data";
    ...
    bChart.refreshChart = function (range, keys) {
        ...
        switch (range) {
            ...
            case RUNTIME_RANGE_WEEK:
                ...
                Ladda.create(document.getElementById('s3Chart' + range)).start();
                $http.post('/boiler_runtime_list/', postData).then(function (res) {
                    ...
                    Ladda.create(document.getElementById('s3Chart' + range)).stop();
                });
                break;
